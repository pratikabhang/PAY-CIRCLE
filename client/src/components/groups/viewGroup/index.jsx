...
const INITIAL_EXPENSE_SHOW_COUNT = 5;
...
export default function ViewGroup() {
    ...
    const [groupExpense, setGroupExpense] = useState({ expense: [], total: 0 });
    ...
    const [showCount, setShowCount] = useState(INITIAL_EXPENSE_SHOW_COUNT);
    const [hasMoreExpenses, setHasMoreExpenses] = useState(true);
    const [expFocus, setExpFocus] = useState(false);
    const [viewSettlement, setViewSettlement] = useState(0); // 0: expenses, 1: group balance, 2: my balance
    const mdUp = useResponsive('up', 'md');

    const findUserSplit = (split) => {
        if (!split || !split.length) return 0;
        return split[0][emailId] || 0;
    };

    const loadMoreExpenses = () => {
        const newCount = showCount + LOAD_MORE_INCREMENT;
        setShowCount(newCount);
        setVisibleExpenses(groupExpense.expense.slice(0, newCount));
        setHasMoreExpenses(newCount < groupExpense.expense.length);
    };
        ...
        const getGroupDetails = async () => {
            ...
            try {
                const [groupResponse, expenseResponse] = await Promise.all([
                    getGroupDetailsService(groupIdJson, setAlert, setAlertMessage),
                    getGroupExpenseService(groupIdJson, setAlertExpense, setAlertExpenseMessage)
                ]);
                ...
                if (expenseResponse) {
                    const expenses = expenseResponse.data?.expense || [];
                    setGroupExpense({
                        expense: expenses,
                        total: expenseResponse.data?.total || 0
                    });
                    setVisibleExpenses(expenses.slice(0, INITIAL_EXPENSE_SHOW_COUNT));
                    setHasMoreExpenses(expenses.length > INITIAL_EXPENSE_SHOW_COUNT);
                }
            } catch (error) {
                console.error('Error fetching group details:', error);
                setAlert(true);
                setAlertMessage('Failed to load group data');
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <Container>
            ...
            <Box sx={{
                ...
                <Grid container spacing={3} mt={'1px'} sx={{ ...(mdUp && { px: 6 }) }}>
                        ...
                        <Stack spacing={2} direction='row' sx={{
                            ...
                            <Box>
                                ...
                                <Typography variant="h5" sx={{ color: (theme) => theme.palette.primary.darker }}>
                                    {currencyFind(group.groupCurrency)} {convertToCurrency(groupExpense.total || 0)}
                                </Typography>
                            </Box>
                        </Stack>
                    </Grid>
                ...
                >
                    ...
                    {viewSettlement === 0 && (
                        <>
                            {alertExpense || groupExpense.expense.length === 0 ? (
                                <Grid container
                                    direction="column"
                                    sx={{
                                        display: 'flex',
                                ...
                                >
                                    <Typography variant="body2" fontSize={18} textAlign={'center'}>
                                        ...
                                        <Link component={RouterLink} to={dataConfig.ADD_EXPENSE_URL + group._id}>
                                            Add Expense
                                        </Link>
                                    </Typography>
                                </Grid>
                                ...
                                <>
                                    <Grid item xs={12} md={expFocus ? 12 : 6}>
                                        <Grid container spacing={2}>
                                            ...
                                            {hasMoreExpenses && (
                                                <Grid item xs={12}>
                                                    <Button onClick={loadMoreExpenses}>View More</Button>
                                                </Grid>
                                            )}
                                        </Grid>
                                    </Grid>
